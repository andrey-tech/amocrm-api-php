...
Документация по REST API **v2** (запросы к API отправляются на /api/v2/) отсутствует на русскоязычной версии сайта,
но пока доступны прямые ссылки на отдельные разделы этой документации.
