...
Документация по REST API **v2** (запросы к API отправляются на /api/v2/) отсутствует в русскоязычной версии сайта,
но пока остаются доступными прямые ссылки на отдельные ее разделы.
